                ...
                <div class="card-body">
                    ...
                    <div class="price-row">
                        ...
                        <div class="product-actions">
                            ${(() => {
                                const vKosiku = kosik.find(p => String(p.id) === String(produkt.id));
                                return vKosiku
                                    ? `<div class="card-qty"><button type="button" onclick="uberProduktPodleId('${escapeJs(produkt.id)}')">−</button><strong>${vKosiku.pocet}</strong><button type="button" onclick="pridejPodleId('${escapeJs(produkt.id)}')">+</button></div>`
...
function pridejPodleId(id) {
    const index = produkty.findIndex(p => String(p.id) === String(id));
    if (index >= 0) pridejDoKosiku(index);
}
function uberProduktPodleId(id) {
    const polozka = kosik.find(p => String(p.id) === String(id));
    if (!polozka) return;
    if (polozka.pocet > 1) polozka.pocet -= 1;
    else kosik = kosik.filter(p => String(p.id) !== String(id));
    ulozKosik();
    vykresliKosik();
    vykresliProdukty();
}
...
function vykresliKosik() {
    ...
    const hodnotaEl = document.getElementById("hodnotaKosiku");
    if (hodnotaEl) hodnotaEl.textContent = formatCena.format(hodnotaKosiku);
